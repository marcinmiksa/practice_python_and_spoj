...
# guess. A naive strategy can be to simply start the guessing at 1, and keep going
# (2, 3, 4, etc.) until you hit the number. But that’s not an optimal guessing strategy.
...
# try to find the optimal strategy! (We’ll talk about what is the optimal one next
# week with the solution.)

import random


def enter_number(number=25):
    return number


def random_number(rand_int=random.randrange(0, 100)):
    return rand_int


def check_number():
    mike_number = enter_number()
    computer_number = random_number()
    counter = 0
    while True:
        if mike_number > computer_number:
            computer_number += 1
        elif mike_number < computer_number:
            computer_number -= 1
        else:
            break
        counter += 1
        print("Mike: {} \t Computer: {}".format(mike_number, computer_number))

    print("\nComputer needed {} steps to guess number!".format(counter))
